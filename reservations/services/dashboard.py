...
from reservations.services.occupancy import peak_day
...
def dashboard_metrics(start_date, end_date):
    ...
    return {
        ...
        "occupancy": {
            "global_utilization_percentage": global_utilization(start_date, end_date),
            "peak_day": peak_day(start_date.year, start_date.month),
            "top_3_rooms": [
                {"id": room.id, "name": room.name}
                for room in top_3_rooms(start_date, end_date)
            ],
        },
        "lifecycle": {
